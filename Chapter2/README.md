# Chapter 2: nORFs – Platform and High-Dimensional Analysis

This chapter covers the nORFs.org platform and the high-dimensional analysis of Novel Open Reading Frames (nORFs) presented in Chapter 2 of the thesis "The Elusive Dynamic of the Genome".

---

## Part A: nORFs.org Platform Code

The code for the nORFs.org platform is maintained in a separate GitHub repository:
...
## Part B: High-Dimensional nORF Analysis Scripts

The R scripts in this directory cover proteochemometric (PCM) feature extraction, statistical analysis, machine learning classification, topology analysis based on predicted structures, and the development of an "nORF Kernel" score.
...
1.  **`01_setup_load_data.R`**:
    ...
    *   **Actions:** Reads nORF GTF, canonical ORF FASTA, loads/generates random sequences, extracts sequence attributes (ID, AA sequence, start codon), optionally checks for missing PDB files, saves loaded data to `chapter2_loaded_data.RData`.
    *   **Key Output:** `chapter2_loaded_data.RData`, `nORFs_all.fasta`, `nORF_start_codon_usage.png`.
...
2.  **`02_pcm_feature_extraction.R`**:
    ...
    *   **Key Input:** `chapter2_loaded_data.RData`.
